...
for seed in ${seeds[@]}; do
  ...
  for baseline in ${baselines[@]}; do
      echo "Running $baseline"
        python unlearn.py --epochs 10 --registered_model resnet18-cifar-10-$seed-retrained --method $baseline --using_CIFAKE True
      fi
  done
done
...
for seed in ${seeds[@]}; do
  ...
  for baseline in ${baselines[@]}; do
      echo "Running $baseline"
      if [ $baseline == "unsir" ]; then
        python unlearn.py --epochs 3 --registered_model vit-cifar-10-$seed-retrained --batch_size 16 --method $baseline --using_CIFAKE True
      else
        python unlearn.py --epochs 3 --registered_model vit-cifar-10-$seed-retrained --batch_size 32 --method $baseline --using_CIFAKE True
      fi
  done
done
...
#############################################
for seed in ${seeds[@]}; do
  python train.py --seed $seed --dataset cifar-100 --model vit --batch_size 64 --epochs 30  --is_early_stop 
  python retrain.py --registered_model vit-cifar-100-$seed-
...
#############################################
# ViT on ImageNet1k (Large-scale unlearning)
# #############################################
...
# In those cases, we decreased the batch size.
