"""
This script performs the Amnesiac Unlearning process of a using the original model.
It loads the original and the retrained model.
It relabels randomly the forget set,
and fine-tunes then fine-tunes the original model with both the retain and the forget set.
...
mlflow.start_run(run_name=f"{model_str}_{dataset}_amnesiac_{str_now}")
...
# Train on retain set and relabeled forget set
model.to(DEVICE)
...
modified_data = []
# Iterate over the forget_dl_relabeld DataLoader
for _, (inp, _) in enumerate(dl["forget"]):
    new_label = random.randint(0, num_classes - 1)
    modified_data.append((inp, new_label))
# Create a new DataLoader with the modified data
forget_dl_relabeld = torch.utils.data.DataLoader(
    modified_data,
    batch_size=batch_size,
    pin_memory=True,
    shuffle=True,
    worker_init_fn=set_work_init_fn(seed),
    num_workers=0,  # Set num_workers to 0 to avoid the resizing issue
)
...
for epoch in tqdm(range(epochs_to_retrain)):
    ...
    for inputs, targets in forget_dl_relabeld:
